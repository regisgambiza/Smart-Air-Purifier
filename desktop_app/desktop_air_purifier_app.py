...
SETTINGS_FILE = str(APP_DIR / "desktop_app_settings.json")
LOG_FILE = str(APP_DIR / "air_purifier_timeseries.csv")
DEBUG_LOG_FILE = str(APP_DIR / "app_debug.log")
CALIBRATION_FILE = str(APP_DIR / "fan_calibration.json")
FILTER_STATE_FILE = str(APP_DIR / "filter_state.json")
FAN_ICON_FILE = str(APP_DIR / "fan_icon.png")
WINDOW_ICON_FILE = str(APP_DIR / "fan_icon.ico")
FAN_ICON_SIZE_PX = 56
...
CONTROL_MODE_CLASSIC = "classic_auto"
CONTROL_MODE_AI = "ai_assist"
...
def sanitize_control_mode(control_mode: str, fallback: str = CONTROL_MODE_CLASSIC) -> str:
    clean = (control_mode or "").strip().lower()
    if clean in CONTROL_MODE_CHOICES:
        return clean
    return fallback if fallback in CONTROL_MODE_CHOICES else CONTROL_MODE_CLASSIC
...
class ConfigManager:
    ...
    def create_config(
        ...
        clean_mode = sanitize_control_mode(control_mode, base.control_mode)
    ...
    def load(self) -> AppConfig:
        ...
        try:
            ...
            config = self.create_config(
                ...
                control_mode=str(raw.get("control_mode", defaults.control_mode)),
...
class App:
    def __init__(self, root: tk.Tk):
        ...
        self.control_mode_var = tk.StringVar(value=sanitize_control_mode(self.config.control_mode))
    ...
    def _control_mode(self) -> str:
        return sanitize_control_mode(self.control_mode_var.get(), self.config.control_mode)

    def _is_manual_mode(self) -> bool:
        return self._control_mode() == CONTROL_MODE_MANUAL

    def _is_auto_mode(self) -> bool:
        return self._control_mode() in {CONTROL_MODE_CLASSIC, CONTROL_MODE_AI}

    def _is_ai_assist_mode(self) -> bool:
        return self._control_mode() == CONTROL_MODE_AI

    def _mode_label(self) -> str:
        mode = self._control_mode()
        return CONTROL_MODE_LABELS.get(mode, "Unknown")

    def _mode_badge_text(self) -> str:
        mode = self._control_mode()
        if mode == CONTROL_MODE_MANUAL:
            return "MANUAL"
        if mode == CONTROL_MODE_CLASSIC:
            return "CLASSIC"
        return "AI"

    def _mode_badge_style(self) -> tuple[str, str]:
        mode = self._control_mode()
        if mode == CONTROL_MODE_MANUAL:
            return "#1e3a8a", "#dbeafe"
        if mode == CONTROL_MODE_CLASSIC:
            return "#7c2d12", "#ffedd5"
    ...
    def _build_ui(self) -> None:
        ...
        self.settings_menu.add_radiobutton(
            label="Classic Auto Mode",
            value=CONTROL_MODE_CLASSIC,
            variable=self.control_mode_var,
            command=self._on_control_mode_change,
        )
        self.settings_menu.add_radiobutton(
            label="AI Assist Mode",
            value=CONTROL_MODE_AI,
            variable=self.control_mode_var,
            command=self._on_control_mode_change,
        )
        ...
        mode_bg, mode_fg = self._mode_badge_style()
        ...
        self.ai_fan_label = ttk.Label(self.fan_card, text="Automatic decision: --", style="Body.TLabel")
        ...
        self.manual_note = ttk.Label(
            self.fan_card,
            text="Manual slider active. ESP should remain in MANUAL mode",
            style="Muted.TLabel",
        )
    ...
    def show_help_dialog(self):
        messagebox.showinfo(
            "Keyboard Shortcuts",
            "Ctrl+R: Refresh now\nCtrl+S: Open settings\nF1: Show this help dialog\nGear menu: refresh, mode selection, autotune, setup, settings, help",
            parent=self.root,
        )
    ...
    def open_settings_window(self):
        ...
        mode_row = len(fields)
        ttk.Label(frame, text="Control Mode", style="Body.TLabel").grid(
            row=mode_row,
            column=0,
            sticky="w",
            pady=6,
        ...
        ttk.Combobox(
            ...
            values=[CONTROL_MODE_MANUAL, CONTROL_MODE_CLASSIC, CONTROL_MODE_AI],
            state="readonly",
        ).grid(row=mode_row, column=1, sticky="ew", pady=6)

        profile_row = mode_row + 1
    ...
    def persist_settings(self) -> bool:
        try:
            ...
            self.control_mode_var.set(new_config.control_mode)
    ...
    def _update_action_states(self):
        ...
        if self.settings_menu is not None:
            per_entry_state = {
                ...
                "Manual Mode": "disabled" if busy else "normal",
                "Classic Auto Mode": "disabled" if busy else "normal",
                "AI Assist Mode": "disabled" if busy else "normal",
    ...
    def _refresh_worker(self):
        ...
        try:
            ...
            if control_mode in {CONTROL_MODE_CLASSIC, CONTROL_MODE_AI}:
                ...
                if control_mode == CONTROL_MODE_AI:
                ...
                if self.fan_controller.should_push(current_speed):
                    try:
                        ...
                        self.health_monitor.record_esp_failure(str(error))
                        self._set_status("Unable to apply automatic fan speed to ESP32.")

                fan_ai_speed = applied_speed
            else:
                self.fan_controller.reset_ai_state()

            allow_ai_text = control_mode == CONTROL_MODE_AI
            advice = self.ai_controller.temperature_advice(
                config,
                esp,
                weather,
                force_fail_safe=(local_fail_safe or not allow_ai_text),
            )
            pollution_comment = self.ai_controller.pollution_comment(
                config,
                air,
                force_fail_safe=(local_fail_safe or not allow_ai_text),
            )
    ...
    def _update_manual_note(self):
        ...
        elif self._is_manual_mode():
            self.manual_note.configure(text="Manual speed slider active. ESP should remain in MANUAL mode")
        elif self._is_ai_assist_mode():
            self.manual_note.configure(text="Manual slider disabled while AI Assist mode is active")
        else:
            self.manual_note.configure(text="Manual slider disabled while Classic Auto mode is active")
    ...
    def _on_control_mode_change(self, trigger_refresh: bool = True):
        mode = self._control_mode()
        self.fan_controller.reset_ai_state()
        self.fail_safe_mode = False
        ...
        self._update_manual_note()
        if mode == CONTROL_MODE_CLASSIC:
            self._set_status("Classic auto control enabled")
            self._set_system_alert("Classic auto control enabled.", "info")
        else:
            self._set_status("AI assist control enabled")
            self._set_system_alert("AI assist mode enabled.", "info")
    ...
    def _force_manual_mode(self):
        ...
        try:
            ...
            self.last_esp_success_ts = time.time()
            self.last_esp = esp
            self._set_status("Manual mode enabled: ESP set to MANUAL")
    ...
    def _update_ui(
        ...
        control_mode = self._control_mode()
        control_mode_text = CONTROL_MODE_LABELS.get(control_mode, "Unknown")
        ...
        if not self._is_manual_mode() or self.autotune_in_progress:
            self.slider.state(["disabled"])
        else:
            self.slider.state(["!disabled"])
        self._update_manual_note()
        ...
        mode_bg, mode_fg = self._mode_badge_style()
        self.summary_mode_label.configure(text=control_mode_text, bg=mode_bg, fg=mode_fg)
        self.mode_badge_label.configure(text=self._mode_badge_text(), bg=mode_bg, fg=mode_fg)
        ...
        if self._is_ai_assist_mode():
            if fan_ai_speed is not None:
                target_text = (
                    self.fan_controller.ai_target_speed
                    if self.fan_controller.ai_target_speed is not None
                    else fan_ai_speed
                )
                source = "AI (fail-safe)" if self.fail_safe_mode else "AI"
                self.ai_fan_label.configure(text=f"AI assist decision: {fan_ai_speed}% (target {target_text}%, {source})")
                self.speed_detail_label.configure(text=f"Target: {target_text}% | Source: {source}")
            else:
                self.ai_fan_label.configure(text="AI assist decision: waiting for refresh")
                self.speed_detail_label.configure(text="Target: -- | Source: AI assist")
        elif self._is_auto_mode():
            if fan_ai_speed is not None:
                target_text = (
                    self.fan_controller.ai_target_speed
                    if self.fan_controller.ai_target_speed is not None
                ...
                source = "Classic (fail-safe)" if self.fail_safe_mode else "Classic"
                self.ai_fan_label.configure(
                    text=f"Classic auto decision: {fan_ai_speed}% (target {target_text}%, {source})"
                )
                self.speed_detail_label.configure(text=f"Target: {target_text}% | Source: {source}")
            else:
                self.ai_fan_label.configure(text="Classic auto decision: waiting for refresh")
                self.speed_detail_label.configure(text="Target: -- | Source: classic")
        else:
            self.ai_fan_label.configure(text="Automatic decision: off")
